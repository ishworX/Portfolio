...
function updatepresenceLoop() {
    theTimeLooper = setInterval(() => {
        ...
        if (!containsOtherTypes) {
            platform = [
                json.active_on_discord_web === true ? "Web, " : "",
                json.active_on_discord_desktop === true ? "Desktop, " : "",
                json.active_on_discord_mobile === true ? "Phone, " : "",
            ];
            document.getElementById("activities").innerHTML =
                '<div class="activity"><div class="other"><ul>' +
                "<li id='type'>No Activities</li><li id='details'>Status: " +
                json.discord_status.toUpperCase() +
                (json.discord_status === "offline" ? "" : " on ") +
                platform.join("") +
                "</li></ul></div></div>";
        ...
        activities.forEach((element) => {
            ...
            if (element["type"] !== 4) {
                ...
                // Checks if the activity already exists, if it does just modify the existing one to not create multiple instances.
                if (exists) {
                ...
                if (element["name"] === "Spotify") {
                    ...
                    var songinfo = [
                        "<li id='spotify-songname'>" + json.spotify["song"] + "</li>",
                        "<li id='spotify-bar'><div class='progress-bar'><div class='fill' style='width: " +
                        percent +
                        "%;'></div><p id='spotify-time'>" +
                        playTime(now) +
                        "/" +
                        playTime(end) +
                        "</p></div></li>",
                        "<li id='spotify-artist'>" +
                        "by " +
                        json.spotify["artist"].split("; ").join(", ") +
                        "</li>",
                        "<li id='spotify-album'>" + "on " + json.spotify["album"] + "</li>",
                    ];

                    div.innerHTML =
                        '<div class="assets-images"><img  class="spotify-thumbnail" onerror=this.src="./assets/img/unknown.png" width="80" height="80" src="' +
                        json.spotify["album_art_url"] +
                        '">' +
                        '<a href="https://open.spotify.com/track/' +
                        json.spotify["track_id"] +
                        '" target="_blank"> <img class="spotifylink-image" width="45" height="45" src="./assets/img/spotifylink.gif"></a>' +
                        "</div>" +
                        '<ul><li id="spotify-ing">' +
                    ...
                    const current_time =
                        element.hasOwnProperty("timestamps") === false
                            ? element["created_at"]
                            : element.timestamps["start"] === undefined
                                ? element["created_at"]
                                : element.timestamps["start"];
                    ...
                    var activityinfo = [
                        "<li id='type'>" +
                        (element["application_id"] === "920907514344779827"
                            ? "ACTIVE ON..."
                            : element["type"] === 0
                                ? "PLAYING..."
                                : element["type"] === 1
                        ...
                        (element["details"] === undefined
                            ? ""
                            : "<li id='details'>" + element["details"] + "</li>"),
                        element["state"] === undefined
                            ? ""
                            : "<li id='state'>" + element["state"] + "</li>",
                        "<li id='time'>" + formatTime(diff) + " elapsed </li>",
                    ];
                    if (element.assets !== undefined) {
                        div.innerHTML =
                            `<div class="assets-images"><img class="large-image" onerror=this.src="./assets/img/unknown.png" width="80" height="80" src="${largeimagelink}">${element.assets["small_image"] === undefined
                                ? ""
                                : `<img class="small-image" width="25" height="25" src="${smallimagelink}">`
                            }</div><div class="other"><ul>${activityinfo.join("")}</ul>` +
                            "</div>";
                    } else if (element.assets === undefined) {
                        div.innerHTML = `<img draggable="false" alt="" width="80" height="80" src="./assets/img/unknown.png"> <div class="other"><ul>${activityinfo.join(
                            ""
                        )}</ul></div>`;
                    }
                }
                ...
                if (exists) currentdiv.appendChild(div);
            }
        });

        // Gets the difference of the current activities and the last, mostly just to remove activities that aren't active anymore.
        let names = [];
        activities.forEach((e) => {
            if (e["type"] !== 4)
                names.push(e["name"].split(" ").join("").toLowerCase());
        });
        var children = [].slice.call(
            currentdiv.getElementsByClassName("activity"),
            0
        );
        ...
        children
            .filter((x) => toremove.includes(x.id))
            .forEach((e) => {
                e.remove();
            });
